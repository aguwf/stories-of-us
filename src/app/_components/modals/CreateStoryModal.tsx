...
const CreateStoryModal = forwardRef<CreateStoryModalRef, CreateStoryModalProps>(
  (props, ref) => {
    ...
    const [openMediaPicker, setOpenMediaPicker] = useState<(() => void) | null>(
      null
    );
    const [pollQuestion, setPollQuestion] = useState("");
    const createPollOption = () => ({ id: crypto.randomUUID(), value: "" });
    const [pollOptions, setPollOptions] = useState<
      Array<{ id: string; value: string }>
    >([createPollOption(), createPollOption()]);
    const cachedDescriptionRef = useRef<string>(
    ...
    useEffect(() => {
      if (selectedStory?.postFormat === "poll" && selectedStory.description) {
        try {
          ...
          setPollOptions(
            parsed?.options?.length ? parsed.options : [{ id: "", value: "" }]
          );
        } catch {
          setPollQuestion(selectedStory.description);
          setPollOptions([{ id: "", value: "" }]);
        }
      } else {
        setPollQuestion("");
        setPollOptions([{ id: "", value: "" }]);
    ...
    const handleScheduleChange = (value: string) => {
      ...
      form.setValue("scheduledPublishTime", parsed);
        ...
        <div className="space-y-2">
          ...
          {pollOptions.length < 6 && (
            <Button
              ...
              size="sm"
              onClick={() =>
                setPollOptions((prev) => [...prev, createPollOption()])
              }
            >
              Add option
            </Button>
        ...
        <DialogContent className="pt-14 max-h-[90vh] overflow-y-auto sm:max-w-[720px]">
          ...
          <form onSubmit={handleFormSubmit}>
            ...
            <Tabs
              value={postFormat}
              onValueChange={(v) =>
                form.setValue("postFormat", v as StoryType["postFormat"])
              }
            >
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid w-full items-center gap-2">
                    ...
                    <UploadV2
                      ...
                      accept="image/*,video/*"
                      onPickerReady={(trigger) =>
                        setOpenMediaPicker(() => trigger)
                      }
                    />
                  </div>
                </div>
